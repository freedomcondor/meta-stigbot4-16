...
int omap4iss_csiphy_acquire(struct iss_csiphy *phy)
{
	...
	mutex_lock(&phy->mutex);
							...
							pr_err("in csiphy acquire, before reset");
	rval = omap4iss_csi2_reset(phy->csi2);
							pr_err("in csiphy acquire, after reset ret = %d",rval);
	if (rval)
		goto done;
	...
	csiphy_lanes_config(phy);
							...
							pr_err("in csiphy acquire, before set power");
	rval = csiphy_set_power(phy, CSI2_COMPLEXIO_CFG_PWD_CMD_ON);
							pr_err("in csiphy acquire, after set power ret = ",rval);
	if (rval)
		goto done;
